...
5. If it stops before collision, adjust the `tap_threshold_dhz`.
...
You can streamline the process by placing the toolhead at the center once.
And upon config restart, trick the machine into thinking that Z is homed.
```
...
absolute `tap_threshold_dhz` value.
