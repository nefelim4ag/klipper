...
check_tap(struct ldc1612 *ld, uint32_t data)
{
    ...
    if (idata > ld->safe_max_value) {
        ld->homing_flags = 0;
        trsync_do_trigger(ld->ts, ld->error_reason);
        output("safe_max_freq: %i", idata);
        return;
    }
    if (ld->prev_sample == 0) {
        ld->prev_sample = idata;
        // Hack to supress initial impulse
        for (int i = 0; i < 32; i++) {
            sosfilt(ld->sf, idata);
        }
    }
    ...
    // Accel would also be positive
    int32_t accel = velocity - ld->prev_vel;
    ld->prev_vel = velocity;
    // Upon tap velocity would decrease and acceleration now negative
    if (accel < ld->min_accel) {
        flags = 0;
        ld->homing_clock = time;
        trsync_do_trigger(ld->ts, ld->trigger_reason);
        output("accel: %i/", accel, ld->min_accel);
    }
