...
stepper_load_next(struct stepper *s)
{
    ...
    if (HAVE_SINGLE_SCHEDULE && s->flags & SF_SINGLE_SCHED) {
        ...
        if (HAVE_FULL_SHORT && s->step_pulse_ticks <= nsecs_to_ticks(250)) {
            s->count = m->count;
...
// Uber fast "double scheduled" step function
uint_fast8_t
stepper_event_full_short(struct timer *t)
{
    ...
    uint32_t min_next_time = curtime + s->step_pulse_ticks;
    s->count--;
    while (unlikely(timer_is_before(timer_read_time(), min_next_time)));
    gpio_out_toggle_noirq(s->step_pin);
...
command_config_stepper(uint32_t *args)
{
    ...
    if (HAVE_EDGE_OPTIMIZATION) {
        ...
        else if (HAVE_FULL_SHORT && s->step_pulse_ticks <= nsecs_to_ticks(250))
