...
def plot_steps(data, oid, skip, diff_level, frequency, mm_per_step):
    ...
    for logname, steps in data:
        ...
        for i in range(count-1):
            ...
            velocities.append(mm_per_step/time)
...
def main():
    ...
    opts.add_option("-l", "--step-length", type="float", dest="step_length",
                    default=0.003125, help="mm per step")
    ...
    fig = plot_steps(data, options.oid, options.skip, options.diff,
                     options.frequency, options.step_length)
