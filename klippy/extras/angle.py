...
class AngleTMCCalibration:
    ...
    def guess_real_resolution(self):
        if self.is_finished:
            ...
            self.printer.lookup_object(self.name).add_client(self.handle_batch)
        self.pause()
        self.move(self.full_steps // 4 * self.full_step_dist)
        self.move(-self.full_steps // 4 * self.full_step_dist)
        toolhead = self.printer.lookup_object('toolhead')
    ...
    def cmd_ANGLE_TMC_CALIBRATE(self, gcmd):
        ...
        min_mres = 256
        while min_mres < fs_resolution:
            min_mres = min_mres >> 1

        # Allow missaligment up to 1 angle sensor step
        misalign = (360 / self.real_resolution) * 1.1
        ...
        ms_angle = fs_4_diff / self.microsteps / 4
        gcmd.respond_info("Ideal step angle: %.4f, allowed drift: %.4f" % (ms_angle, misalign))
        quant = self.mscnt_quant
        positions = [i for i in range(self.mscnt_min, 1024, quant)]
        ...
        while tries:
            ...
            self.move((self.full_steps + 1) * self.full_step_dist)
            ...
            ideal_angle = prev_angle + ms_angle * angle_dir
            min_dist = 1
            max_dist = 0
            ms_diffs = []
            ms_dist = []
            ...
            for pos in positions:
                ...
                change = distance / ideal_ms / 4
                pos = pos % 256
                if distance < -misalign:
                    for i in range((pos - self.mscnt_quant // 2), (pos+1)):
                        sin_value[i] += change / (pos + 1 - i)
                    up += 1
                    not_matched += 1
                elif distance > misalign:
                    for i in range((pos - self.mscnt_quant // 2), (pos+1)):
                        sin_value[i] -= change / (pos + 1 - i)
                    down += 1
                    not_matched += 1
            ...
            y_new = p(x_new)
            sin_new = self.mslut_normalize(y_new)
            history.append({
                "sin": sin_new,
                "stddev": 360,
            })
            ...
            self._force_disable()
            if self.driver == "tmc5160":
                self.tmc.fields.set_field("recalibrate", 1)
            for i in range(0, 8):
