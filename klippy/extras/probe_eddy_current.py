...
class EddyCalibration:
    ...
    def cmd_CALIBRATION_OFFSET(self, gcmd):
        is_update = gcmd.get('OFFSET', False)
        if not is_update:
...
class EddyGatherSamples:
    ...
    def _check_sensor_messages(self):
        while self._sensor_messages and self._probe_requests:
            ...
            try:
                ...
                res = cb(measures, start_time, end_time, *args)
    ...
    def _add_probe_request(self, cb, start_time, end_time, *args):
    ...
    def _await_sensor_messages(self):
        ...
        while self._probe_requests:
            ...
            if mcu.is_fileoutput():
                ...
                dummy_pr = self._create_probe_result(0., [0., 0., 0.])
    ...
    def _measures_to_z(self, measures):
        # Find average sensor frequency in measurements
        if not measures:
            raise self._printer.command_error(
        ...
        sensor_z = self._calibration.freq_to_height(freq_avg)
        if sensor_z <= -OUT_OF_RANGE or sensor_z >= OUT_OF_RANGE:
            raise self._printer.command_error(
                "probe_eddy_current sensor not in valid range")
        return sensor_z
    def _create_probe_result(self, sensor_z, toolhead_pos):
    ...
    # Handle typical descend until trigger probe requests
    def _analyze_descend(self, measures, start_time, end_time, toolhead_pos):
        sensor_z = self._measures_to_z(measures)
        return self._create_probe_result(sensor_z, toolhead_pos)
    def note_descend_probe(self, start_time, end_time, toolhead_pos):
        self._add_probe_request(self._analyze_descend, start_time, end_time,
                                toolhead_pos)
    # Handle "scan" and "rapid_scan" probe request
    def _analyze_scan(self, measures, start_time, end_time, pos_time):
        sensor_z = self._measures_to_z(measures)
